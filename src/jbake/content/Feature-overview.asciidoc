...
Since version 1.1.2 Gretty can link:Web-app-filtering.html[filter files in "src/main/webapp"] the same way as processResources can filter files in "src/main/resources". Gretty sources contain https://github.com/akhikhl/gretty/tree/master/examples/filterWebapp[example program] showing the needed configuration.
